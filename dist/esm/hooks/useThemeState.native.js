...
var useThemeState = function (props) {
  ...
  var parentId = useContext(ThemeStateContext);
  if (!parentId && !isRoot) {
    throw new Error(process.env.NODE_ENV === "development" ? `${MISSING_THEME_MESSAGE}
...
Looked for theme${props.name ? ` "${props.name}"` : ""}${props.componentName ? ` (component: ${props.componentName})` : ""}, but no parent theme context was found (parentId: ${parentId}).` : MISSING_THEME_MESSAGE);
  ...
  var getSnapshot = function () {
    ...
    var isSchemeOnlyChange = isIos && getSetting("fastSchemeChange") && local && parentState && local.scheme !== parentState.scheme && getThemeBaseName(local.name) === getThemeBaseName(parentState.name);
